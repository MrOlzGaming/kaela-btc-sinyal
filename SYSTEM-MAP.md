...
| Forced-flow/liquidity radar (Fase 1) | 🟡 RADAR INFO, BELUM SINYAL | `actionableLiquidityRadar.js` | 21 Sep 2026, filosofi Olan ("jangan tanya BTC ke mana, tanya uang siapa yang paling rentan dipaksa bergerak"). Gabung histori cluster (`liquidationListener.js`) + crowding live (`marketSentiment.js`) + burst real-time (`liquidation-events.jsonl`). Trigger >=$300rb burst/30 menit -> WA broadcast, cooldown 2 jam. Threshold BELUM divalidasi backtest -- Fase 2 (kumpul histori respons harga) & Fase 3 (backtest+masuk Anomaly Scanner) nunggu hasil Fase 1 |
