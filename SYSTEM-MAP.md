...
| Miner pool tracking (siapa mining apa) | 🟡 FONDASI, BELUM JADI SINYAL | `minerPools.js`+`minerPoolResearchLog.js` | Deteksi tag pool via scriptSig coinbase (GRATIS+PASTI, numpang blok whale digest, zero API tambahan). BARU rekap "siapa mining berapa blok" -- BELUM bisa jawab "miner jual/nggak" (butuh clustering wallet payout tiap pool, langkah lanjutan kalau worth dikerjain) |
