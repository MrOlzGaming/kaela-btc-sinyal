...
| BUG-KAELATRADE-0005 | 2026-09-14 | **KRITIS** — `run-vultr-executor.sh` macet TOTAL 6+ jam (19:32-01:41 WITA), NOL siklus 15-menit jalan sama sekali. Ketauan dari GEJALA gak langsung: Olan lapor "PnL Harian 0%" di web (data pool gak ke-update karena executor stuck, bukan pasar diem) | 18 dari 26 panggilan `node` di script ini gak pernah dikasih `timeout` — kalau salah satu (paling mungkin `multiAccountExecutor.js`, panggilan exchange API) hang, flock (fd 200) ketahan SELAMANYA sampai proses itu mati sendiri | `5a90110` — semua panggilan node wajib `timeout -k 10 <detik>` (pola yang manjur buat fix `git fetch` bulan lalu), plus `checkExecutorStuck.js` (pengawas baru, cron 1 menit terpisah, japri Olan kalau >=3 siklus berturut2 gak ada "Run mulai") | `regressionTests.js` (test `detectStuck`, ground truth pola waktu insiden asli) | ✅ CLOSED |
