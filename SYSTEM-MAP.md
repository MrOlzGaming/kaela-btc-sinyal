...
| Forced-flow/liquidity radar (Fase 1) | 🟡 RADAR INFO, BELUM SINYAL, DIPANTAU | `actionableLiquidityRadar.js` | 21 Sep 2026, filosofi Olan ("jangan tanya BTC ke mana, tanya uang siapa yang paling rentan dipaksa bergerak"). DUA jalur: (A) burst -- >=$300rb likuidasi/30 menit, cooldown 2 jam; (B) imbalance -- funding ekstrem + cluster historis notable (>=$500rb, 3% harga) deket, cooldown 8 jam, LEBIH SERING nembak (permintaan Olan "jangan mangkrak"). Tiap sinyal ke-log ke `actionable-liquidity-signal-log.json` (bahan Fase 2). Threshold BELUM divalidasi backtest, status "DIPANTAU beberapa hari" -- JANGAN ubah threshold/lanjut Fase 2-3 tanpa diminta Olan |
